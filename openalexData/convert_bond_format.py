        ...
        try:
            ...
            for idx in range(min(abstract_index_2_text.keys()), max(abstract_index_2_text.keys())+1):
                if idx not in abstract_index_2_text:
                    raise Exception('missing idx in abstract_inverted_index')
...
def generate_author_data():
    ...
    with open('./src/total/train_authors.json', 'w') as f:
        f.write(str(author_gt_data))
    print(f'dumped ./src/total/train_authors.json')
    return pub_set
